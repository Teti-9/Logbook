...
const UserRouter = (userService, refreshTokenService) => {
   ...
   *     tags: [User (JWT Implementation not Better Auth.)]
   *     parameters:
   *       - in: path
   *         name: id
   *         required: true
  ...
  router.post("/logins", async (req, res) => {
    ...
    if (loggedUser) {
      res.cookie(
        ...
        refreshTokenService._cookieOptions(),
      );
    }
  ...
  router.post("/logouts", async (req, res) => {
    ...
    const logoutUser = await refreshTokenService.logoutUser(
      req.cookies.refreshToken,
    );
    ...
    if (logoutUser) {
      res.clearCookie(
        "refreshToken",
        refreshTokenService._cookieOptions({ clear: true }),
      );
    }
